    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="bg-sage text-white text-sm px-4 py-2 rounded-lg hover:bg-[#3D6A4A] transition-colors font-medium"
        >
          + Nueva cita
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-sage-bg border border-blue-100 rounded-xl p-4 space-y-3 mb-6">
          ...
          <div>
            ...
            <select value={patientId} onChange={e => setPatientId(e.target.value)}
              className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-cream-raised focus:outline-none focus:ring-[3px] focus:ring-sage-bg">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleSave} disabled={saving || !patientId || !datetime}
              className="flex-1 py-2 bg-sage text-white rounded-lg text-sm font-medium hover:bg-[#3D6A4A] disabled:opacity-50">
      ...
      {list.length === 0 && !showForm && (
        <div className="text-center py-16 text-graphite-subtle">
      ...
      {Object.entries(grouped).map(([day, appts]) => (
        <section key={day} className="mb-6">
          <h3 className="text-xs font-semibold text-graphite-subtle uppercase tracking-wide mb-2 capitalize">{day}</h3>
          <ul className="space-y-2">
            {appts.map(a => (
              <li key={a.id} className={`bg-cream-raised border border-border rounded-xl px-4 py-3 flex items-center justify-between ${a.status === 'atendida' ? 'opacity-50' : ''}`}>
                <div>
                  <p className={`text-sm font-medium ${a.status === 'atendida' ? 'line-through text-graphite-subtle' : 'text-graphite'}`}>
                    {a.patient?.name ?? 'Paciente'}
                  </p>
                  {a.reason && <p className="text-xs text-graphite-subtle">{a.reason}</p>}
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-xs text-graphite-subtle font-mono">
