...
type Order = { id: string; exams: string; notes: string | null; created_at: string }

type Props = { orders: Order[]; patientId: string; doctorId: string }

export default function OrdenesClient({ orders: initial, patientId, doctorId }: Props) {
  ...
  async function handleSave() {
    ...
    const { error, data } = await supabase
      .from('exam_orders')
      .insert({ patient_id: patientId, doctor_id: doctorId, exams, notes: notes || null })
      .select()
      .single()
    ...
    <div className="space-y-4">
      {!showForm ? (
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleSave} disabled={saving || !exams.trim()} className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50">
      ...
      {list.map(o => (
        <div key={o.id} className="bg-white border border-gray-100 rounded-xl p-4">
          ...
          <pre className="text-sm text-gray-900 whitespace-pre-wrap font-mono">{o.exams}</pre>
